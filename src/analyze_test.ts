...
Deno.test({
  ...
  fn() {
    ...
    assertEqualsTokens(analyze(["cmd", "--option"], spec), [
      $command<Command, Option>(0, 0, 3, "cmd", spec.subcommands![0]),
...
Deno.test({
  ...
  fn() {
    ...
    assertEqualsTokens(analyze(["long-name"], spec), [
      $command<Command, Option>(0, 0, 9, "long-name", spec.subcommands![0]),
    ]);
    // Matches one exactly, even when it is the prefix of another command
    assertEqualsTokens(analyze(["abc"], spec), [
      $command<Command, Option>(0, 0, 3, "abc", spec.subcommands![3]),
    ]);
    ...
    assertEqualsTokens(analyze(["long-n"], spec), [
      $command<Command, Option>(0, 0, 6, "long-n", spec.subcommands![0]),
    ]);
    // should match only one
    assertEqualsTokens(analyze(["n"], spec), [
      $command<Command, Option>(0, 0, 1, "n", spec.subcommands![0]),
    ]);
  },
});
